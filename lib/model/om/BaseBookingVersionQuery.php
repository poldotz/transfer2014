 ...
 * @method BookingVersionQuery orderByReduced($order = Criteria::ASC) Order by the reduced column
 ...
 * @method BookingVersionQuery groupByReduced() Group by the reduced column
 ...
 * @method BookingVersion findOneByReduced(int $reduced) Return the first BookingVersion filtered by the reduced column
 ...
 * @method array findByReduced(int $reduced) Return BookingVersion objects filtered by the reduced column
...
abstract class BaseBookingVersionQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `BOOKING_DATE`, `YEAR`, `NUMBER`, `ADULT`, `REDUCED`, `CHILD`, `CONTACT`, `RIF_FILE`, `CUSTOMER_ID`, `VEHICLE_TYPE_ID`, `IS_CONFIRMED`, `CREATED_AT`, `UPDATED_AT`, `VERSION`, `VERSION_CREATED_AT`, `VERSION_CREATED_BY`, `ARRIVAL_IDS`, `ARRIVAL_VERSIONS`, `DEPARTURE_IDS`, `DEPARTURE_VERSIONS` FROM `booking_version` WHERE `ID` = :p0 AND `VERSION` = :p1';
        try {
    ...
    public function filterByReduced($reduced = null, $comparison = null)
    {
        if (is_array($reduced)) {
            ...
            if (isset($reduced['min'])) {
                $this->addUsingAlias(BookingVersionPeer::REDUCED, $reduced['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($reduced['max'])) {
                $this->addUsingAlias(BookingVersionPeer::REDUCED, $reduced['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(BookingVersionPeer::REDUCED, $reduced, $comparison);
    }
