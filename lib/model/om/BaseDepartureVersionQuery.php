 ...
 * @method DepartureVersionQuery orderByCost($order = Criteria::ASC) Order by the cost column
 ...
 * @method DepartureVersionQuery groupByCost() Group by the cost column
 ...
 * @method DepartureVersion findOneByCost(string $cost) Return the first DepartureVersion filtered by the cost column
 ...
 * @method array findByCost(string $cost) Return DepartureVersion objects filtered by the cost column
...
abstract class BaseDepartureVersionQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `BOOKING_ID`, `DAY`, `HOUR`, `PICK_UP`, `DEPARTURE_TIME`, `FLIGHT`, `COST`, `NOTE`, `PAYMENT_METHOD_ID`, `LOCALITY_FROM`, `LOCALITY_TO`, `DRIVER_ID`, `VEHICLE_ID`, `CANCELLED`, `CREATED_AT`, `UPDATED_AT`, `VERSION`, `VERSION_CREATED_AT`, `VERSION_CREATED_BY`, `BOOKING_ID_VERSION` FROM `departure_version` WHERE `ID` = :p0 AND `VERSION` = :p1';
        try {
    ...
    public function filterByCost($cost = null, $comparison = null)
    {
        if (is_array($cost)) {
            $useMinMax = false;
            if (isset($cost['min'])) {
                $this->addUsingAlias(DepartureVersionPeer::COST, $cost['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($cost['max'])) {
                $this->addUsingAlias(DepartureVersionPeer::COST, $cost['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(DepartureVersionPeer::COST, $cost, $comparison);
    }
