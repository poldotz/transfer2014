 ...
 * @method DepartureQuery orderByCost($order = Criteria::ASC) Order by the cost column
 ...
 * @method DepartureQuery groupByCost() Group by the cost column
 ...
 * @method Departure findOneByCost(string $cost) Return the first Departure filtered by the cost column
 ...
 * @method array findByCost(string $cost) Return Departure objects filtered by the cost column
...
abstract class BaseDepartureQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT `ID`, `BOOKING_ID`, `DAY`, `HOUR`, `PICK_UP`, `DEPARTURE_TIME`, `FLIGHT`, `COST`, `NOTE`, `PAYMENT_METHOD_ID`, `LOCALITY_FROM`, `LOCALITY_TO`, `DRIVER_ID`, `VEHICLE_ID`, `CANCELLED`, `CREATED_AT`, `UPDATED_AT`, `VERSION`, `VERSION_CREATED_AT`, `VERSION_CREATED_BY` FROM `departure` WHERE `ID` = :p0';
        try {
    ...
    public function filterByCost($cost = null, $comparison = null)
    {
        if (is_array($cost)) {
            $useMinMax = false;
            if (isset($cost['min'])) {
                $this->addUsingAlias(DeparturePeer::COST, $cost['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($cost['max'])) {
                $this->addUsingAlias(DeparturePeer::COST, $cost['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(DeparturePeer::COST, $cost, $comparison);
    }
