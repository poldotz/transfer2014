...
class PluginsfGuardFormResetPassword extends BaseForm
{
  public function configure()
  {
    ...
    $this->setValidators(array(
      'password'         => new sfValidatorString(array('min_length' => 8), array('min_length' => 'Password  troppo corta ( minimo %min_length% caratteri ).', 'required' => 'Campo Obbligatorio')),
      'password_confirm' => new sfValidatorString(array(), array('required' => 'Campo Obbligatorio')),
    ));

    $this->validatorSchema->setPostValidator(
      new sfValidatorSchemaCompare('password', sfValidatorSchemaCompare::EQUAL, 'password_confirm', array(), array('invalid' => 'Le due password inserite devono essere uguali'))
    );
