...
abstract class sfBaseTaskLoggerTask extends sfBaseTask
{
  ...
  protected function logEnd()
  {
    ...
    protected function setCountProcessed($count = 0){
        $this->task->setCountProcessed($count);
    }

    protected function setCountNotProcessed($count = 0){
        $this->task->setCountNotProcessed($count);
  ...
  protected function configure()
  {
    ...
    $this->addOptions(array(
      ...
      new sfCommandOption('application', null, sfCommandOption::PARAMETER_REQUIRED, 'Application used', 'backend'),
    ));
