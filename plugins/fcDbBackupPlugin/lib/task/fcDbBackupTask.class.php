...
class fcDbBackupTask extends sfBaseTaskLoggerTask
{
    ...
    protected function configure()
    {
        ...
        $this->namespace        = 'cron';
        $this->name             = 'dbBackup';
        $this->briefDescription = 'Effettua un dump del database. Triggers the automatic archiving system.';
        ...
        [php symfony cron:dbBackup|INFO]
EOF;
    }
    ...
    protected function initializeConfig()
    {
        ...
        $validPath = dirname(__FILE__)."/../../../..".$validPath;

        //-- Path to RDBMS executable
        $pathToExec             = sfConfig::get('app_fcDbBackupPlugin_pathToExec', '');
        $this->pathToExecutable = ($pathToExec == '') ? '' : (preg_match('#(\/|\\\)$#', $pathToExec) ? $pathToExec : $pathToExec . DIRECTORY_SEPARATOR);
    ...
    {
        $toDel = preg_match('#(\/|\\\)#', $file) ? basename($file) : $file;
    ...
    {
        $toHandle = preg_match('#(\/|\\\)#', $file) ? basename($file) : $file;
    ...
    protected function doProcess($arguments = array(), $options = array())
    {
        ...
        exec($command,$results,$res);
        $this->logSection('backup', sprintf('Backup effettuato il %s', date('d m Y')));
        $this->setCountProcessed(1);
        $this->setOk();
